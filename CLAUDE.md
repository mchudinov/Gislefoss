...
**Current state: server-side persistent-agent migration implemented in code and Bicep; live deploy pending.** The agent runtime is wired: `src/Agent/Foundry/MeteorologistAgentFactory.cs` obtains the `AIAgent` server-side, by name (`AsAIAgent` + `AgentReference`; an asst_ id is rejected by the SDK name regex), `src/Agent/Foundry/FoundryAgentRunner.cs` adapts it to the `IFoundryAgentRunner` port, DI is wired in `src/Agent/ServiceCollectionExtensions.cs`, and `src/Web/Program.cs` no longer reads the persona at boot, fetching the agent lazily on first use; tests are green. The Blazor home page (`src/Web/Components/Pages/Home.razor`) is still the default "Hello, world!" stub, but a functional weather chat UI exists at `src/Web/Components/Pages/Chat.razor`. What exists: a complete persona at `personas/gislefoss.md` (a top-level repo asset, sibling to `src/` and `infra/`, embedded into the agent at deploy time via Bicep `loadTextContent` in `infra/main.bicep`; no longer copied into the Web output), plus an `Agent` class library and an `Agent.Tests` xUnit project (Phases 1–5) holding value types, `AgentOptions`, run-outcome inspection, a conversation facade, the factory, the runner adapter, and DI — all built and green. The Bicep infrastructure exists under `infra/` (Foundry account/project/model deployment + block Guardrail, Log Analytics + App Insights, a single-replica Container App, and role assignments), built for the Responses-path deployment. The project is **migrating to a server-side persistent Foundry agent**: the persona stays in git (`personas/gislefoss.md`) and will be pushed to the agent on each deploy by `infra/modules/agent.bicep` (a deployment-script that upserts the agent by name with the persona on each deploy); the app retrieves it by name (config key `Settings:Agent:AgentName`, default Gislefoss). Capability hosts (`foundry.bicep`), the provisioning UAMI (`identity.bicep`), split RBAC (`roles.bicep` + `roles-app.bicep`), and a `deployApp` toggle in `main.bicep` are implemented and az-bicep-build clean; only the live `az deployment` remains (its `upsert-agent.py` Python SDK surface and capability-host preview APIs are deploy-verify items). See the migration plan at `docs/superpowers/plans/2026-06-07-meteorologist-agent-foundry-server-side-migration.md`. `Azure.AI.OpenAI` 2.1.0 is referenced and `Settings` already has an `AzureOpenAI` config block. Treat `docs/idea.md` as the requirements spec, not a description of existing code.
