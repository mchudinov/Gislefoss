...
> **Revision note (2026-06-07 — server-side agent migration):** The project is migrating back to a **server-side persistent Foundry agent**. The app drives the agent by id via `PersistentAgentsClient.GetAIAgentAsync(AgentId)` (config `Settings:Agent:AgentId`) — the persona is no longer read at runtime. The persona file `src/Web/personas/gislefoss.md` remains the git source; Bicep embeds and pushes it to the server-side agent on each deploy. See [2026-06-07-meteorologist-agent-foundry-server-side-migration.md](2026-06-07-meteorologist-agent-foundry-server-side-migration.md).

**Architecture:** A `net10.0` class library `Agent` holds all wiring. In the current (pre-migration) implementation the persona is read from `personas/gislefoss.md` and passed as `instructions` to `AIProjectClient.AsAIAgent(...)`, yielding an in-process `AIAgent` (`ChatClientAgent`). Under the migration (see the 2026-06-07 plan), the app instead retrieves a **server-side persistent agent** by id via `PersistentAgentsClient.GetAIAgentAsync(AgentId)` — the persona lives server-side and is no longer read by the app at runtime. A `MeteorologistAgentFactory` reads + validates the persona and builds the agent; it is registered as an **eagerly-constructed singleton**, so a missing/empty persona **fails the app at boot**. Domain logic (run-outcome classification, the conversation facade) is TDD'd against the `IFoundryAgentRunner` **port**; the Foundry SDK lives only in the thin runner adapter + factory. The Blazor `Web` app consumes the library via `AddMeteorologistAgent(...)`. Protection is fully platform-side (deployment Guardrail, `block` action); the app only reads outcomes.
